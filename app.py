...
def is_reference_head(para):
    """
    判斷段落是否為參考文獻開頭（APA、APA_LIKE、IEEE 或編號格式）
    - 已新增對 "數字." 開頭格式的判斷
    """
    # 規則 1：判斷標準 APA 格式 (YYYY)
    if find_apa(para):
        return True
    ...
    # ==================== [ ✨ 新增的修改區域 START ✨ ] ====================
    # 規則 3：偵測數字編號開頭的格式 (例如 "1." 或 "15.")
    # 為避免誤判文章中的普通編號列表 (例如 "1. 前言")，增加一個長度限制，
    # 只有較長的段落才視為參考文獻。
    if re.match(r"^\d{1,3}[.)、．]\s+", para.strip()):
        if len(para.strip()) > 25: # 參考文獻通常不會太短
            return True
    # ==================== [ ✨ 新增的修改區域 END ✨ ] ======================

    # 規則 4：判斷類 APA 格式, YYYY.
